...
there are several instances where the digitisation of features by
...
applications are made at a continental or global scale. Consequently,
...
with low toxicity larvicide to reduce the malarial mosquito population. 
...
axis (*a*) and a blue-yellow axis (*b*) [@rathore2012colour; @baldevbhai2012color; @pandy2017rgblab]. A Comparison between the original RGB colour Image and the *L\*A\*B* transformed image can be seen in Figure 1.  The *L\*A\*B* colour space can help account for
...
coordinates of the selected point are retrieved and a square
...
Pixels with a lower colour distance are
of a similar colour.
...
Pixels which can be of the same region can be defined where the colour distance, summed with the spatial distance have a value greater than the user defined threshold as described in figure 5. Where this condition is not met, pixels are set to set to 0  and considered to be no data values. 
